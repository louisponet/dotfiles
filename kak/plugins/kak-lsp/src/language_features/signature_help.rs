...
fn editor_signature_help(
    ...
    let active_signature_ = result.active_signature.unwrap_or(0);

    let active_signature_ = match result.signatures.get(active_signature_ as usize) {
        Some(active_signature_) => active_signature_,
        None => return,
    };

    let contents = result.signatures.iter().map(|active_signature| {
        ...
        let mut tcontents = active_signature.label.clone();
        if let Some(range) = parameter_range {
            if range[0] >= tcontents.len() || range[1] >= tcontents.len() {
                warn!("invalid range for active parameter");
            } else {
                let (left, tail) = tcontents.split_at(range[0]);
                let (param, right) = tail.split_at(range[1] - range[0]);
                tcontents = escape_kakoune_markup(left)
                    + "{+b}"
